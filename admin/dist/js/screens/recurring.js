...
const REC_DAYS = [
  ...
  { key: "wednesday", label: "\xC7ar" },
  { key: "thursday", label: "Per" },
  { key: "friday", label: "Cum" },
  { key: "saturday", label: "Cmt" },
  { key: "sunday", label: "Paz" }
];
const recLocalDateStr = (d) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
window.RecurringLessonModal = function RecurringLessonModal({ clubId, onClose, onCreated }) {
  ...
  const [recDurationType, setRecDurationType] = useState("monthly");
  const [recSelectedCourt, setRecSelectedCourt] = useState(null);
  const [submitting, setSubmitting] = useState(false);
  const [recStudentMode, setRecStudentMode] = useState("member");
  ...
  useEffect(() => {
    (async () => {
      const [cRes, ctRes] = await Promise.all([
        sb.from("club_coaches").select("id, full_name, hourly_rate, individual_coach_id, coach_pay_rate").eq("club_id", clubId).eq("is_active", true).order("full_name"),
        sb.from("courts").select("id, court_number, court_type, hourly_rate").eq("club_id", clubId).eq("is_active", true).order("court_number")
      ]);
  ...
  const searchStudents = async (q) => {
    ...
    try {
      const { data } = await sb.from("profiles").select("id, full_name").eq("user_type", "player").ilike("full_name", `%${q.trim()}%`).limit(6);
  ...
  const searchCustomers = async (q) => {
    if (q.trim().length < 2) {
      setRecCustomerResults([]);
      return;
    }
    try {
      const { data } = await sb.from("club_customers").select("id, full_name, phone, user_id").eq("club_id", clubId).eq("is_active", true).or(`full_name.ilike.%${q.trim()}%,phone.ilike.%${q.trim()}%`).limit(8);
  ...
  const resetStudent = () => {
    ...
    setRecSearchQuery("");
    setRecSearchResults([]);
    setRecCustomerSearch("");
    setRecCustomerResults([]);
    setRecGuestName("");
    setRecStudentDisplayName("");
    setRecCustomerId(null);
  };
  useEffect(() => {
    const hasStudent = recStudent?.id || recStudentDisplayName;
    if (!hasStudent) {
      setRecAvailablePackages([]);
      setRecSelectedPackageId(null);
      setRecUsePackage(false);
      return;
    }
    (async () => {
      ...
      try {
        ...
        const baseFilters = (q) => q.eq("club_id", clubId).in("payment_status", ["paid", "pending"]).eq("status", "active").or(`expiry_date.is.null,expiry_date.gt.${now}`).order("created_at", { ascending: false });
        ...
        if (recStudent?.id) {
          ...
          q = baseFilters(q);
          if (coachAuthId || clubCoachId) q = applyCoachFilter(q);
          queries.push(q);
        }
        ...
        const merged = [];
        for (const { data } of results) for (const row of data || []) if (!seen.has(row.id)) {
          seen.add(row.id);
          merged.push(row);
        }
  ...
  const useSession = async (playerPackageId, coachAuthId, sessionDate, lessonId, notes) => {
    ...
    const { error: sErr } = await sb.from("lesson_package_sessions").insert({
      player_package_id: playerPackageId,
      lesson_id: lessonId ?? null,
      coach_id: coachAuthId,
    ...
    if (payoutMode === "per_session" && (pkg || isCustom)) {
      ...
      if (ccRes.data && coachAmount > 0) {
        const { error: seErr } = await sb.from("coach_earnings").insert({
          coach_id: ccRes.data.id,
          club_id: cid,
          coach_name: ccRes.data.full_name ?? "Antren\xF6r",
          student_name: plRes.data?.full_name ?? "\xD6\u011Frenci",
          amount: coachAmount,
          date: sessionDate,
  ...
  const handleSubmit = async () => {
    if (!recTitle.trim()) {
      alert("L\xFCtfen ders ba\u015Fl\u0131\u011F\u0131 girin.");
      return;
    }
    if (!recCoach) {
      alert("L\xFCtfen bir hoca se\xE7in.");
      return;
    }
    const studentLabel = recStudentMode === "guest" ? recGuestName.trim() : recStudentMode === "customer" ? recStudentDisplayName : recStudent?.full_name ?? recSearchQuery.trim();
    if (!studentLabel) {
      alert("L\xFCtfen \xF6\u011Frenci bilgisi girin.");
      return;
    }
    if (recSelectedDays.length === 0) {
      alert("L\xFCtfen en az bir g\xFCn se\xE7in.");
      return;
    }
    const [sh, sm] = recStartTime.split(":").map(Number);
    const [eh, em] = recEndTime.split(":").map(Number);
    if (isNaN(sh) || isNaN(eh) || eh * 60 + em <= sh * 60 + sm) {
      alert("Ge\xE7erli bir ba\u015Flang\u0131\xE7 ve biti\u015F saati girin.");
      return;
    }
    ...
    for (let week = 0; week < weekCountForCheck; week++) {
      for (const dayKey of recSelectedDays) {
        ...
        if (coachAuthId) {
          const { data: coachLessons } = await sb.from("lessons").select("id").eq("coach_id", coachAuthId).neq("status", "cancelled").lt("start_time", endIso).gt("end_time", startIso);
        ...
        if (recStudent?.id) {
          ...
          if (bRes.data && bRes.data.length > 0 || lRes.data && lRes.data.length > 0) {
            const msg = `${dateStr}: \xD6\u011Frenci bu saatte me\u015Fgul`;
            if (!conflictWarnings.includes(msg)) conflictWarnings.push(msg);
          }
        }
        if (recSelectedCourt) {
...
...ve ${conflictWarnings.length - 6} \xE7ak\u0131\u015Fma daha` : ""}`;
      if (!confirm(`\u26A0\uFE0F \xC7ak\u0131\u015Fmalar Tespit Edildi
    ...
    try {
      ...
      for (let week = 0; week < weekCount; week++) {
        for (const dayKey of recSelectedDays) {
          ...
          if (lessonDate < /* @__PURE__ */ new Date()) continue;
          const dateStr = recLocalDateStr(lessonDate);
          const startIso = `${dateStr}T${recStartTime}:00+03:00`;
          const endIso = `${dateStr}T${recEndTime}:00+03:00`;
          if (coachAuthId) {
            const { data: existingConflict } = await sb.from("lessons").select("id").eq("coach_id", coachAuthId).neq("status", "cancelled").lt("start_time", endIso).gt("end_time", startIso).limit(1);
          ...
          const lessonData = {
            ...
            location: recSelectedCourt ? `Kort ${recSelectedCourt.court_number}` : "Kul\xFCp",
      ...
      if (lessonsCreated.length === 0) {
        alert(`Hi\xE7bir ders olu\u015Fturulamad\u0131. Se\xE7ilen t\xFCm tarihler (${skippedDates.length} adet) mevcut derslerle \xE7ak\u0131\u015F\u0131yor.`);
        return;
      }
...
${packagedCount} ders paketten kar\u015F\u0131land\u0131. Paket bitti\u011Fi i\xE7in ${normalPriceLessons.length} ders normal fiyatla eklendi.`;
      } else if (recUsePackage && normalPriceLessons.length > 0 && packagedCount === 0) {
        successMsg += `

Pakette kalan ders olmad\u0131\u011F\u0131 i\xE7in t\xFCm dersler normal fiyatla eklendi.`;
      }
      if (skippedDates.length > 0) successMsg += `

${skippedDates.length} tarih \xE7ak\u0131\u015Fma nedeniyle atland\u0131.`;
      onCreated?.();
      onClose?.();
      alert(successMsg);
    } catch (e) {
  ...
  return /* @__PURE__ */ React.createElement(
    Modal,
    {
      ...
      sub: "Se\xE7ilen g\xFCnlere, ay boyunca otomatik ders olu\u015Ftur",
      onClose,
      footer: /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("button", { className: "btn btn-ghost btn-sm", onClick: onClose }, "Vazge\xE7"), /* @__PURE__ */ React.createElement("button", { className: "btn btn-pri btn-sm", onClick: handleSubmit, disabled: !canSubmit }, /* @__PURE__ */ React.createElement("span", { className: "material-icons", style: { fontSize: 16 } }, "repeat"), submitting ? " Olu\u015Fturuluyor\u2026" : " Olu\u015Ftur"))
    },
    /* @__PURE__ */ React.createElement("div", { className: "fields", style: { gap: 14 } }, /* @__PURE__ */ React.createElement(Field, { label: "Hoca" }, coaches.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { color: "var(--text-2)", fontSize: 13 } }, "Kul\xFCbe kay\u0131tl\u0131 aktif hoca bulunamad\u0131.") : /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 6 } }, coaches.map((c) => {
      ...
      return /* @__PURE__ */ React.createElement(
        "button",
        {
          key: c.id,
          type: "button",
          onClick: () => setRecCoach(active ? null : c),
          style: {
            display: "flex",
            alignItems: "center",
            gap: 6,
        ...
        /* @__PURE__ */ React.createElement("span", { className: "material-icons", style: { fontSize: 15, color: active ? "#fff" : "var(--brand-navy)" } }, "person"),
        c.full_name,
        active && /* @__PURE__ */ React.createElement("span", { className: "material-icons", style: { fontSize: 15 } }, "check_circle")
      );
    }))), /* @__PURE__ */ React.createElement(Field, { label: "Ders Ba\u015Fl\u0131\u011F\u0131" }, /* @__PURE__ */ React.createElement("input", { value: recTitle, placeholder: "\xD6rn: Tenis Dersi", onChange: (e) => setRecTitle(e.target.value) })), /* @__PURE__ */ React.createElement(Field, { label: "\xD6\u011Frenci" }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", borderRadius: 8, overflow: "hidden", border: "1px solid var(--border)", marginBottom: 8 } }, [{ v: "member", l: "\xDCye" }, { v: "customer", l: "M\xFC\u015Fteri" }, { v: "guest", l: "Misafir" }].map((m) => /* @__PURE__ */ React.createElement(
      "button",
      {
        key: m.v,
        type: "button",
        onClick: () => {
          setRecStudentMode(m.v);
          resetStudent();
        },
        style: {
          flex: 1,
          ...
          border: "none",
          cursor: "pointer",
          background: recStudentMode === m.v ? "var(--brand-navy)" : "transparent",
          color: recStudentMode === m.v ? "#fff" : "var(--text-1)"
        }
      },
      m.l
    ))), selectedStudentName && recStudentMode !== "guest" ? /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, background: "#EEF2FF", borderRadius: 10, padding: "9px 12px", border: "1px solid #C7D2FE" } }, /* @__PURE__ */ React.createElement("span", { className: "material-icons", style: { color: "var(--brand-navy)", fontSize: 16 } }, "verified_user"), /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontWeight: 700, fontSize: 13, color: "var(--brand-navy)" } }, selectedStudentName), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: resetStudent, style: { background: "none", border: "none", cursor: "pointer", color: "#EF4444", padding: 0 } }, /* @__PURE__ */ React.createElement("span", { className: "material-icons", style: { fontSize: 16 } }, "close"))) : recStudentMode === "guest" ? /* @__PURE__ */ React.createElement("input", { value: recGuestName, placeholder: "Misafir ad\u0131 yaz\u0131n", onChange: (e) => setRecGuestName(e.target.value) }) : recStudentMode === "member" ? /* @__PURE__ */ React.createElement("div", { style: { position: "relative" } }, /* @__PURE__ */ React.createElement(
      "input",
      {
        value: recSearchQuery,
        placeholder: "CourtyCLUB \xFCyesi ara\u2026",
        onChange: (e) => {
          setRecSearchQuery(e.target.value);
          searchStudents(e.target.value);
        }
      }
    ), recSearchResults.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: "100%", left: 0, right: 0, zIndex: 30, background: "#fff", border: "1px solid var(--border)", borderRadius: 10, boxShadow: "0 4px 12px rgba(0,0,0,0.1)", overflow: "hidden", marginTop: 4 } }, recSearchResults.map((st, idx) => /* @__PURE__ */ React.createElement(
      "div",
      {
        key: st.id,
        onMouseDown: () => {
          setRecStudent({ id: st.id, full_name: st.full_name });
          setRecStudentDisplayName(st.full_name);
          setRecCustomerId(null);
          setRecSearchQuery("");
          setRecSearchResults([]);
        },
      ...
      /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontWeight: 600 } }, st.full_name),
      /* @__PURE__ */ React.createElement("span", { className: "material-icons", style: { fontSize: 14, color: "#22C55E" } }, "verified")
    )))) : /* @__PURE__ */ React.createElement("div", { style: { position: "relative" } }, /* @__PURE__ */ React.createElement(
      "input",
      {
        value: recCustomerSearch,
        placeholder: "M\xFC\u015Fteri ad\u0131 veya telefon ara\u2026",
        onChange: (e) => {
          setRecCustomerSearch(e.target.value);
          searchCustomers(e.target.value);
        }
      }
    ), recCustomerResults.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: "100%", left: 0, right: 0, zIndex: 30, background: "#fff", border: "1px solid var(--border)", borderRadius: 10, boxShadow: "0 4px 12px rgba(0,0,0,0.1)", overflow: "hidden", marginTop: 4 } }, recCustomerResults.map((c, idx) => /* @__PURE__ */ React.createElement(
      "div",
      {
        key: c.id,
        onMouseDown: () => {
          setRecStudentDisplayName(c.full_name);
          setRecCustomerId(c.id);
          setRecCustomerSearch("");
          setRecCustomerResults([]);
          if (c.user_id) setRecStudent({ id: c.user_id, full_name: c.full_name });
        },
      ...
      /* @__PURE__ */ React.createElement("span", { className: "material-icons", style: { fontSize: 15, color: "var(--brand-navy)" } }, "person"),
      /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontWeight: 600 } }, c.full_name),
      c.phone && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "var(--text-2)" } }, c.phone)
    ))))), recStudent && recAvailablePackages.length > 0 && /* @__PURE__ */ React.createElement(Field, { label: "Paketten D\xFC\u015F", hint: "Bu dersleri \xF6\u011Frencinin aktif paketinden say" }, /* @__PURE__ */ React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 13 } }, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: recUsePackage, onChange: (e) => setRecUsePackage(e.target.checked) }), "Aktif paketten d\xFC\u015F"), recUsePackage && (recLoadingPackages ? /* @__PURE__ */ React.createElement("div", { style: { color: "var(--text-2)", fontSize: 12, marginTop: 6 } }, "Paketler y\xFCkleniyor\u2026") : /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6, marginTop: 6 } }, recAvailablePackages.map((pkg) => {
      ...
      return /* @__PURE__ */ React.createElement(
        "button",
        {
          key: pkg.id,
          type: "button",
          onClick: () => setRecSelectedPackageId(pkg.id),
          style: {
            display: "flex",
            alignItems: "center",
            gap: 8,
            ...
            borderRadius: 8,
            cursor: "pointer",
            border: `1px solid ${sel ? "var(--brand-navy)" : "var(--border)"}`,
    ...
    }))), /* @__PURE__ */ React.createElement(Field, { label: "Ders \xDCcreti (\u20BA)" }, /* @__PURE__ */ React.createElement(
      "input",
      {
        type: "number",
        min: "0",
        placeholder: recUsePackage && recSelectedPackageId ? "Paketten kar\u015F\u0131lan\u0131yor" : "0",
        ...
        style: recUsePackage && recSelectedPackageId ? { background: "#F1F5F9", color: "var(--text-2)" } : void 0
      }
    )), /* @__PURE__ */ React.createElement(Field, { label: "S\xFCre" }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8 } }, [{ v: "monthly", l: "Ayl\u0131k (4 hafta)", i: "date_range" }, { v: "3months", l: "3 Ayl\u0131k (12 hafta)", i: "event_repeat" }].map((opt) => {
      ...
      return /* @__PURE__ */ React.createElement(
        "button",
        {
          key: opt.v,
          type: "button",
          onClick: () => setRecDurationType(opt.v),
          style: {
            flex: 1,
            display: "flex",
            alignItems: "center",
        ...
        /* @__PURE__ */ React.createElement("span", { className: "material-icons", style: { fontSize: 16, color: active ? "#fff" : "var(--text-2)" } }, opt.i),
        opt.l
      );
    }))), /* @__PURE__ */ React.createElement(Field, { label: "Notlar (Opsiyonel)" }, /* @__PURE__ */ React.createElement("textarea", { rows: 2, value: recNotes, placeholder: "Ders hakk\u0131nda notlar\u2026", onChange: (e) => setRecNotes(e.target.value), style: { resize: "vertical" } })), courts.length > 0 && /* @__PURE__ */ React.createElement(Field, { label: "Kort Se\xE7imi (Opsiyonel)" }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 6 } }, courts.map((court) => {
      ...
      return /* @__PURE__ */ React.createElement(
        "button",
        {
          key: court.id,
          type: "button",
          onClick: () => setRecSelectedCourt(active ? null : court),
          style: {
            padding: "8px 11px",
            borderRadius: 9,
            cursor: "pointer",
        ...
        /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700 } }, "Kort ", court.court_number),
        /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: active ? "rgba(255,255,255,0.8)" : "var(--text-2)" } }, court.court_type, " \xB7 ", court.hourly_rate, "\u20BA/sa")
      );
    }))))
  );
};
