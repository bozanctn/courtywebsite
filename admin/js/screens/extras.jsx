...
function MyProgramScreen({ clubId, setScreen, clubProfile }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [bkRes, lessonRes, manualRes, closureRes] = await Promise.all([
        sb.from('bookings')
          .select('id,start_time,end_time,status,payment_status,total_amount,user_id,court_id,club_customer_id,player_name')
      ...
      // player_name alanı en güvenilir kaynak — RLS kısıtını aşar
      (bkRes.data || []).forEach(b => {
        if (b.player_name) playerMap_.set(b.id, b.player_name);
      });
      // Müşteri rezervasyonları: hâlâ isim yoksa club_customers'dan al
  ...
  const reconcilePackageOnEdit = async ({ editId, orig, coachId, studentName, playerId, customerId, date }) => {
    ...
    // ── A) Eski paket seansını + kredisini geri al (ÖĞRENCİ-BAZLI, koç opsiyonel) ──
    // Önce yüklemede öğrenci-bazlı hesaplanmış seans id'sini kullan; yoksa öğrenci+tarih ile taze
    // doğrula. Eski koç+tarih araması null-koç seansını kaçırıyor ve yanlış öğrencinin kredisini
    // geri alabiliyordu.
    ...
    if (orig.is_package && !oldSessId) {
      const { data: sess } = await sb.from('lesson_package_sessions')
        ...
        .eq('session_date', orig.date).is('lesson_id', null);
      ...
      const hit = (sess || []).find(s => {
        ...
        return coachOk && (
          (orig.club_customer_id && o.club_customer_id && orig.club_customer_id === o.club_customer_id) ||
          (orig.player_id && o.player_id && orig.player_id === o.player_id) ||
          (orig.student_name && o.manual_player_name && _nrmR(orig.student_name) === _nrmR(o.manual_player_name))
        );
      });
      if (hit) { oldSessId = hit.id; oldPkgId = hit.player_package_id; }
    }
    ...
    if (customerId)    queries.push(baseQ().eq('club_customer_id', customerId));  // Fix B: değişmez id bağı
    const results = await Promise.all(queries);
    const seen = new Set();
    ...
    if (orig.is_package) {
      ...
      if (personChanged || coachChangedPkg) {
        const msg = await reconcilePackageOnEdit({
          editId, orig, coachId,
          studentName: payload.student_name, playerId: payload.player_id || null,
          customerId: payload.club_customer_id || null, date: payload.date,
        });
        if (msg) alert(msg);
      }
            ...
            try {
              ...
              if (coachEarning > 0) {
                splitPromises.push(sb.from('coach_earnings').insert({
                  ...
                  court_fee:          0,
                  date:               lsForm.date,
                  description:        `Özel ders (pay) - ${lsForm.student_name || 'Öğrenci'} - ${lsForm.start_time}`,
                  payment_status:     'unpaid',
                  collected_by_coach: false,
                  court_fee_settled:  false,
                }));
              ...
              if (clubEarning > 0) {
                splitPromises.push(sb.from('club_finances').insert({
                  ...
                  category:    'Özel Ders Geliri',
                  amount:      clubEarning,
                  description: `${coachRec.full_name} - ${lsForm.student_name || 'Öğrenci'} - Özel Ders`,
                  date:        lsForm.date,
                }));
              }
            ...
            <div style={{ overflowY:'auto', flex:1, display:'flex', flexDirection:'column', gap:0, padding:'16px 20px' }}>
              ...
              <div style={{ display:'flex', gap:8, marginBottom:12 }}>
                <button
                  ...
                  onClick={() => { setLsPriceMode('normal'); setLsForm(prev => ({...prev, payment_status:'unpaid'})); }}
                >Özel Fiyat</button>
                <button
              ...
              {bookingForm.courtId && (() => {
                ...
                return (
                  <div style={{ background:'#F8FAFC', borderRadius:14, padding:'16px 18px', border:'1px solid var(--border)' }}>
                    ...
                    {bookingPriceOverride && parseFloat(bookingPriceOverride) !== calcAmt && (
